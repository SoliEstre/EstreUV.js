...
// ═══════════════════════════════════════════════════════════════════════════
// EstreUV.js integration — Lit-based micro-Rimwork tiles inside EstreUI article
//
// scripts/estreuv-tiles.js (module) 가 <estreuv-*> custom element 등록 +
// window._estreuv = { intent, bridge, alias } 노출 + 'estreuv:ready' dispatch.
// 본 classic main.js 는 EstreUI page lifecycle 을 자식 EstreUV tile 로 전파 +
// onOpen 시점에 intent provider 부착 (race 회피: bringPage 전에 _estreuvReady await).
// ═══════════════════════════════════════════════════════════════════════════

const _estreuvReady = window._estreuv
    ? Promise.resolve()
    : new Promise(resolve => window.addEventListener("estreuv:ready", () => resolve(), { once: true }));

let _estreuvIntent = null;       // window._spikeIntent 와 동일 참조 노출 (콘솔 호환)
let _estreuvLifecycle = null;    // window._spikeLifecycle 와 동일 참조 노출

class HomePageHandler extends EstrePageHandler {

    onBring(handle) { super.onBring?.(handle); console.log("[estreuv-app] HomePageHandler.onBring", handle?.pid); this.dispatchToTiles(handle, "onBring"); }

    onOpen(handle) {
        super.onOpen?.(handle);
        console.log("[estreuv-app] HomePageHandler.onOpen", handle?.pid);
        this.dispatchToTiles(handle, "onOpen");
        ...
        if (articleRoot && window._estreuv && !_estreuvIntent) {
            ...
            // child → article-level event-up bridge
            articleRoot.addEventListener("intent-update", (e) => {
                _estreuvIntent.update(e.detail.patch);
                console.log("[estreuv-app] intent-update via event:", e.detail.patch);
            });
            console.log("[estreuv-app] intent provider + lifecycle bridge wired to articleRoot:", articleRoot);
        }
    }

    onShow(handle) { super.onShow?.(handle); console.log("[estreuv-app] HomePageHandler.onShow", handle?.pid); this.dispatchToTiles(handle, "onShow"); }
    onFocus(handle, isFirstFocus) { super.onFocus?.(handle, isFirstFocus); console.log("[estreuv-app] HomePageHandler.onFocus", handle?.pid, "isFirstFocus:", isFirstFocus); this.dispatchToTiles(handle, "onFocus", isFirstFocus); }
    onBlur(handle, isFinalBlur) { super.onBlur?.(handle, isFinalBlur); console.log("[estreuv-app] HomePageHandler.onBlur", handle?.pid, "isFinalBlur:", isFinalBlur); this.dispatchToTiles(handle, "onBlur", isFinalBlur); }
    onHide(handle) { super.onHide?.(handle); console.log("[estreuv-app] HomePageHandler.onHide", handle?.pid); this.dispatchToTiles(handle, "onHide"); }
    onClose(handle) { super.onClose?.(handle); console.log("[estreuv-app] HomePageHandler.onClose", handle?.pid); this.dispatchToTiles(handle, "onClose"); }
    onRelease(handle) { super.onRelease?.(handle); console.log("[estreuv-app] HomePageHandler.onRelease", handle?.pid); this.dispatchToTiles(handle, "onRelease"); }

    /** EstreUI page lifecycle 을 자식 EstreUV tile 의 동명 메서드로 전파 */
    dispatchToTiles(handle, hookName, ...extraArgs) {
        const articleRoot = handle?.article ?? handle?.element ?? document.querySelector('[data-article-id="main"][data-static="1"]');
        if (!articleRoot) return;
        articleRoot.querySelectorAll('[data-estreuv]').forEach(($tile) => {
            if (typeof $tile[hookName] === "function") {
                try { $tile[hookName](handle, ...extraArgs); }
                catch (err) { console.error(`[estreuv-app] tile.${hookName} threw:`, err); }
            }
        });
    }
}
...
class AppPagesProvider {
    ...
    "home" = HomePageHandler;   // EstreUV tile lifecycle 전파 + intent provider 부착 (위 클래스 정의)
...
$(document).ready((e) => setTimeout(async _ => {

    //<= to do implement my own initializing

    // EstreUV module (scripts/estreuv-tiles.js) 도착 보장 — onOpen 안의 intent provider 부착 분기가 통과되도록
    await _estreuvReady;
    ...
    console.log("[estreuv-app] booted. Try in DevTools after 'home' opens:");
    console.log("  window._spikeIntent.update({ darkMode: 'dark' })");
    console.log("  document.querySelector('estreuv-dark-mode-tile').cycle()");
    console.log("  window._spikeLifecycle.onShow({stub:true}); window._spikeLifecycle.onShow({stub:true});  // race dedup");
}, 1));
